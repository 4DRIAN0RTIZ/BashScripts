...
function AptLog {
	{
	aptlog=$(grep -n "Start-Date:" /var/log/apt/history.log | 
    while read -r line
    do
    ...
    done) > echo $result > result} | whiptail --gauge "Historial de apt" --textbox result 10 60 --stdout
}

	# aptlog=$(cat /var/log/apt/history.log | grep "Start-Date" | wc -l)
	# 			result="Number of apt-get commands: $aptlog"
	# echo $result > result
	# } | whiptail --gauge "Getting data ..." 6 60 0
...
do
CHOICE=$(
whiptail --title "SpecOs" --menu "Elige la opcion" 16 100 9 \
	"1)" "Historial de apt"   \
	"2)" "T since last boot."  \
